...
■ シナリオプリセット・URL共有
FTバブルやサテライトなど、よくある状況をワンタップで再現できるプリセットを収録。作った状況はURLで共有することもできます。
...
Scenario presets & URL sharing
Recreate common spots — final-table bubble, satellites, and more — with one tap, and share any scenario you build via a URL.
...
device except if the user explicitly taps "Share scenario via URL," which
encodes the scenario itself (not personal data) into a URL the user chooses
to share. See the App Privacy answers submitted alongside this build for
details.
...
- ダーク/ライートどちらのテーマで撮るかは実装側のデフォルトテーマに合わせる（現状ダークテーマ優先を想定）。
