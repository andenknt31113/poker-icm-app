...
- ネイティブ機能は最小限 (`@capacitor/status-bar` でテーマ追従、`@capacitor/share` で共有シート)。
...
- **練習機能 (Practice モード)**: 単なる情報表示ではなく、押し引き判定
  ...
  インタラクティブな学習機能を持つ。
- **ネイティブ UI 調整**: テーマ切替時のステータスバー同期
  (`@capacitor/status-bar`)、ネイティブ共有シート
  (`@capacitor/share`) など、単なる WebView 表示ではなく OS ネイティブ
  UI と連携する調整を行っている。
...
- Native integration: status bar style follows the in-app theme toggle,
  and the share button uses the native iOS share sheet.
...
- インタラクティブな練習機能: 押し引き判定・レンジ当てなどのクイズに
  ...
  閲覧に留まらない機能を備えています。
- ネイティブ連携: テーマ切替に連動したステータスバーのスタイル追従、
  ネイティブ共有シートを利用した共有機能など、OS ネイティブ UI との
  連携も行っています。
